...
/*/{Protheus.doc} XRELAT03
Relatório Frete Mundial por Produto com FWMSPrinter (linhas zebradas)
...
Static Function fMontaRel()
    ...
    Private nColProd     := nColIni + 070
    Private nColUnd      := nColFin - 060 - 060 - 060 - 040
    Private nColQnt      := nColFin - 060 - 060 - 060
    Private nColFreteUnd := nColFin - 060 - 060
    Private nColFreteTot := nColFin - 060
    ...
    cQryRel += "PRODUTO, " + CRLF
    cQryRel += "UND, " + CRLF
    cQryRel += "SUM(QUANTIDADE) AS QUANTIDADE, " + CRLF
    cQryRel += "FRETE, " + CRLF
    cQryRel += "FRETE*SUM(QUANTIDADE) AS FRETETOTAL " + CRLF
    cQryRel += "FROM " + CRLF
    cQryRel += "(SELECT " + CRLF
    cQryRel += "F2_DOC AS DOCUMENTO, " + CRLF
    cQryRel += "A1_NREDUZ AS CLIENTE, " + CRLF
    cQryRel += "B1_DESC AS PRODUTO, " + CRLF
    cQryRel += "D2_UM AS UND, " + CRLF
    cQryRel += "D2_QUANT AS QUANTIDADE, " + CRLF
    cQryRel += "A4_NREDUZ AS TRANSPORTADORA, " + CRLF
    cQryRel += "D2_ITEM AS ORDEM, " + CRLF
    cQryRel += "GW1_NRROM AS ROMANEIO, " + CRLF
    ...
    cQryRel += "CLIENTE, PRODUTO, UND, FRETE " + CRLF
    ...
    If ! QRY_REL->(EoF())
        ...
        If lJob
            ...
            cArquivo := "XRELAT03_job_" + dToS(dDataGer) + "_" + StrTran(cHoraGer, ':', '-') + ".pdf"
        ...
        While ! QRY_REL->(EoF())
            ...
            oPrintPvt:SayAlign(nLinAtu, nColCli,        Alltrim(QRY_REL->CLIENTE),               oFontDet,  070,    10, , PAD_LEFT,   )
            oPrintPvt:SayAlign(nLinAtu, nColProd,       Alltrim(QRY_REL->PRODUTO),               oFontDet,  200,    10, , PAD_LEFT,   )
            oPrintPvt:SayAlign(nLinAtu, nColUnd,        Alltrim(QRY_REL->UND),                   oFontDet,  040,    10, , PAD_CENTER,   )
            oPrintPvt:SayAlign(nLinAtu, nColQnt,        ALLTRIM(TRANSFORM(QRY_REL->QUANTIDADE,"@E 999,999,999.99")),oFontDet,  060,    10, , PAD_RIGHT,   )
            oPrintPvt:SayAlign(nLinAtu, nColFreteUnd,   "R$ "+ALLTRIM(TRANSFORM(QRY_REL->FRETE,"@E 999,999,999.99")),     oFontDet,  060,    10, , PAD_RIGHT,   )
            oPrintPvt:SayAlign(nLinAtu, nColFreteTot,   "R$ "+ALLTRIM(TRANSFORM(QRY_REL->FRETETOTAL,"@E 999,999,999.99")),oFontDet,  060,    10, , PAD_RIGHT,   )
            nLinAtu += nEspLin
        ...
        EndDo
        //Imprime a soma
        oPrintPvt:SayAlign(nLinAtu, nColFreteTot, "R$ "+ALLTRIM(TRANSFORM(nTotal,"@E 999,999,999.99"))  ,oFontDetN,  060,    10, nCorForte, PAD_RIGHT,   )
        oPrintPvt:SayAlign(nLinAtu, nColQnt, "Frete total:"  ,oFontDetN,  060,    10, nCorForte, PAD_RIGHT,   )

        //Imprime a soma com 5%
        oPrintPvt:SayAlign(nLinAtu+nEspLin, nColFreteTot, "R$ "+ALLTRIM(TRANSFORM(nTotal/0.95,"@E 999,999,999.99"))  ,oFontDetN,  060,    10, nCorVerme, PAD_RIGHT,   )
        oPrintPvt:SayAlign(nLinAtu+nEspLin, nColQnt, "Frete total com 5%:"  ,oFontDetN,  100,    10, nCorVerme, PAD_RIGHT,   )
...
Static Function fImpCab()
    ...
    nLinAtu += 010
    oPrintPvt:SayAlign(nLinAtu+05, nColCli,         "Cliente",       oFontMin,  070,    10, nCorCinza, PAD_LEFT,   )
    oPrintPvt:SayAlign(nLinAtu+05, nColProd,        "Produto",       oFontMin,  200,    10, nCorCinza, PAD_LEFT,   )
    oPrintPvt:SayAlign(nLinAtu+05, nColUnd,         "Und",           oFontMin,  040,    10, nCorCinza, PAD_CENTER,   )
    oPrintPvt:SayAlign(nLinAtu+05, nColQnt,         "Quantidade",    oFontMin,  060,    10, nCorCinza, PAD_RIGHT,   )
//  oPrintPvt:SayAlign(nLinAtu+10, nColQnt,         "Descrição",     oFontMin,  100,    10, nCorCinza, PAD_LEFT,   )
    oPrintPvt:SayAlign(nLinAtu+05, nColFreteUnd,    "Frete Und",     oFontMin,  060,    10, nCorCinza, PAD_RIGHT,   )
    oPrintPvt:SayAlign(nLinAtu+05, nColFreteTot,    "Frete total",   oFontMin,  060,    10, nCorCinza, PAD_RIGHT,   )
//  oPrintPvt:SayAlign(nLinAtu+10, nColFreteTot,    "Descrição",     oFontMin,  100,    10, nCorCinza, PAD_LEFT,   )
